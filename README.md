...
Lisp interpreter written in [continuation passing style](https://en.wikipedia.org/wiki/Continuation-passing_style) and [trampolined through thunk-returning](https://en.wikipedia.org/wiki/Trampoline_(computing)#:~:text=As%20used%20in,programming%20languages.), allowing the interpreter to maintain its own call stack on the heap and execute recursive calls of arbitary depth.
